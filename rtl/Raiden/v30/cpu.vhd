...
-- ####################################################################################
                     ...
                     if (aluop /= ALU_OP_NOTHING) then
                        case (aluop) is
                           ...
                           when ALU_OP_ADD | ALU_OP_INC | ALU_OP_ADC =>
                              ...
                              if (opsize = 1) then
                                 regs.FlagOvf <= (source1Val(7) xor result(7)) and (source2Val(7) xor result(7));
                              else
                                 regs.FlagOvf <= (source1Val(15) xor result(15)) and (source2Val(15) xor result(15));
                              end if;
                           ...
                           when ALU_OP_SUB | ALU_OP_CMP | ALU_OP_DEC | ALU_OP_SBB =>
                              ...
                              if (opsize = 1) then
                                 regs.FlagOvf <= (source1Val(7) xor source2Val(7)) and (source1Val(7) xor result(7));
                              else
                                 regs.FlagOvf <= (source1Val(15) xor source2Val(15)) and (source1Val(15) xor result(15));
                              end if;
                           ...
                           when ALU_OP_MULI => 
                              ...
                              if (opsize = 1) then
                                 ...
                                 -- fix IMUL: CF/OF=1 se il risultato non entra in signed 8-bit,
                                 -- cioe' se sign-extend(result(7:0)) /= result(15:0).
                                 if (resize(signed(result32(7 downto 0)), 16) /= signed(result32(15 downto 0))) then
                                    regs.FlagCar <= '1'; regs.FlagOvf <= '1';
                                 end if;
                              else
                                 ...
                                 -- fix IMUL: CF/OF=1 se il risultato non entra in signed 16-bit,
                                 -- cioe' se sign-extend(result(15:0)) /= result(31:0). Il vecchio "/= 0" sbagliava per negativi.
                                 if (resize(signed(result32(15 downto 0)), 32) /= signed(result32(31 downto 0))) then
                                    regs.FlagCar <= '1'; regs.FlagOvf <= '1';
                                 end if;
                              end if;
                     ...
                     if (newZero = '1') then
                        -- fix: per opsize=1 il flag Zero va valutato solo sul byte basso.
                        -- ADD/ADC/INC 8-bit lasciano result17(8)=carry in result(15:8):
                        -- testare i 16 bit dava ZF=0 quando AL=0 con carry (es. INC 0xFF).
                        if (opsize = 1) then
                           if (result(7 downto 0) = 0) then regs.FlagZer <= '1'; else regs.FlagZer <= '0'; end if;
                        else
...
-- ####################################################################################
                     ...
                     case (opcode) is
                        ...
                        when OP_MULADJUST =>
                           if (opstep = 0 and ce = '1') then
                              ...
                              -- fix: AAM (D4 imm8) usa l'immediato come divisore, non 10 fisso.
                              DIVdivisor <= resize(signed('0' & fetch1Val(7 downto 0)), 33);
                           elsif (ce = '1') then
                              exeDone := '1';
                              if (fetch1Val = 0) then
                                 irqrequest <= '1';
                                 ...
                                 -- fix: AAM setta i flag sul risultato AL (= resto), non su AX intero.
                                 -- ZF = (rem = 0). Il vecchio "rem=0 AND quot=0" sbagliava per AL multipli di 10.
                        ...
                        when OP_DIVADJUST =>
                           -- fix: AAD (D5 imm8) usa l'immediato dell'istruzione, non 10 fisso.
                           result8 := regs.reg_ax(7 downto 0) + resize(regs.reg_ax(15 downto 8) * fetch1Val(7 downto 0), 8);
            ...
            if (clearPrefetch = '1') then
               ...
               -- fix bug score intermittente: clearPrefetch azzera il buffer/count
               -- ma NON consumePrefetch. Il ciclo dopo, prefetchCount(0) - consumePrefetch
               -- (residuo del branch) underflow -> prefetchCount wrap a valore grande ->
               -- fetch di opcode/immediati STALE dal buffer vuoto -> es. mov dx,0xa legge
               -- un immediato sbagliato -> moltiplicatore score errato -> score ×100.
               consumePrefetch  <= 0;
            end if;
